...
Description:
  UPDATE: This $0 script is a copy of build.sh but without the vSphere/vCenter requirements. It is intended to run against standalone
  ESXi servers.
...
Usage:
  ...
  Ensure that the environment variables are set:
    ...
    05) "_VCENTER_CLUSTER" --          NOT SET. Used to specify the vSphere cluster.
    06) "_VCENTER_DATACENTER" --       NOT SET. Used to specify the vSphere datacenter.
    07) "_VCENTER_SERVER" --           NOT SET. Used to specify the ESXi or vSphere server name and the path of where the VM should be pushed to.
    08) "_VCENTER_TEMPLATE_FOLDER" --  NOT SET. Used to specify the vCenter folder to store the VM.
...
upload_new_image () {
  ...
  if [[ ! -f ${CORE_OS_VMDK_FILE} ]] ; then
    ...
    ovftool --skipManifestCheck --disableVerification --noSSLVerify --diskMode=thin --datastore=${_ESXI_DATASTORE} --overwrite ${CORE_OS_OVF_FILE} vi://${_VCENTER_USER}:${_VCENTER_USER_PASSWORD}@${OVF_PUSH_TARGET}
  fi
}
